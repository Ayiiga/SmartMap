...
2. **Root Directory:** `smartmap` (required — monorepo)
...
| `NEXT_PUBLIC_APP_NAME` | `Smart Map` |
...
| Project name? | **smartmap** (or link `prj_fueH7SVjqymh14dzrvCGoHIKSfcg`) |
| Directory? | **./** |
...
**Project ID (Smart Map):** `prj_fueH7SVjqymh14dzrvCGoHIKSfcg`
...
Config file: `smartmap/vercel.json` (PWA cache headers + security headers).
