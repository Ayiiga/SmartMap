...
function placeNameFromLabel(label?: string): string {
  if (!label) return "this location";
  const cleaned = label
    .replace(/^📍\s*/, "")
    .replace(/^Current Location\s*·\s*/i, "")
    .trim();
  return cleaned || "this location";
}
...
export function YouAreHerePanel({ coordinates, label, className }: YouAreHerePanelProps) {
  ...
  const placeName = placeNameFromLabel(label);
  ...
  return (
    <div
      ...
      role="status"
      aria-label={`You are here at ${placeName}`}
    >
      <div className="px-4 pb-4 pt-5">
        <p className="text-xs font-semibold uppercase tracking-wide text-[#10B981]">
          Arrived
        </p>
        <h2 className="mt-1 font-display text-2xl font-extrabold text-[#F8FAFC]">
          You are here at {placeName}
        </h2>
        <p className="mt-1 text-sm text-[#94A3B8]">
          Pick a different destination to get turn-by-turn directions.
        </p>
                  ...
                  >
                    ...
                    <div className="flex shrink-0 gap-1.5">
                      <Link
                        ...
                        className="rounded-xl border border-[#2A3A5C] bg-transparent px-2.5 py-1.5 text-[10px] font-bold text-[#94A3B8]"
                      >
                        Map
                      </Link>
                      <Link
                        ...
                        className="inline-flex items-center gap-1 rounded-xl bg-gradient-to-r from-[#3B82F6] to-[#1E5EB8] px-2.5 py-1.5 text-[10px] font-bold text-[#F8FAFC]"
                      >
